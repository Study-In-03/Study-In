        ...
        {studies.length > 0 ? (
          ...
          <div className="flex flex-col gap-2 p-4 bg-background border border-gray-100 rounded-[12px]">
            {studies.map((study) => {
              ...
              return (
                <div key={study.id} className="flex gap-3 py-2 border-b last:border-0 border-gray-100 cursor-pointer" onClick={() => navigate(`/study/${study.id}`)}>
                  <img
                  ...
                  <div className="flex flex-col justify-center overflow-hidden">
                    <div className="flex items-center gap-1 font-medium">
                      <IconSpeaker className={`w-3 h-3 shrink-0 ${STATUS_COLOR[statusName] ?? "text-gray-500"}`} />
                      <span className={`text-sm ${STATUS_COLOR[statusName] ?? "text-gray-500"}`}>
                        {statusName === "모집 중" ? "모집 중!" : statusName}
                      </span>
                    </div>
                    <p className="text-base font-medium truncate text-surface">{study.title}</p>
                  </div>
                </div>
              );
            })}
